...
static inline void CPU_Push(CPU *cpu, uint8_t value) {
    ...
    cpu->sp--; // Stack grows downwards, uint8_t handles wrap-around for SP
}
...
inline void CPU_SetFlag(CPU *cpu, uint8_t flag, bool value) {
    if (value)
        cpu->status |= flag;
    else
        cpu->status &= ~flag;
...
inline bool CPU_GetFlag(CPU *cpu, uint8_t flag) {
...
static inline uint16_t CPU_ADDR_REL(CPU *cpu, bool *page_crossed) { *page_crossed = false; uint16_t offset_addr = cpu->pc++; int8_t offset = (int8_t)BUS_Read(cpu->nes, offset_addr); return cpu->pc + offset; }
...
static void CPU_OP_NOP(CPU *cpu, uint16_t address, uint8_t *cycles_ref); // Unofficial NOPs
...
void CPU_Reset(CPU *cpu) {
    ...
    cpu->x = 0;
    cpu->y = 0;
    cpu->sp = 0xFD; // Stack pointer starts at $FD ($01FD after wrapping)
    cpu->status = CPU_FLAG_UNUSED | CPU_FLAG_INTERRUPT; // Start with I and U flags set
    cpu->pc = BUS_Read16(cpu->nes, 0xFFFC); // Read reset vector
    cpu->total_cycles = 7; // Reset sequence takes 7 cycles
}
...
static void CPU_OP_NOP(CPU *cpu, uint16_t address, uint8_t *cycles_ref) { (void)cpu; (void)address; (void)cycles_ref; /* Do nothing */ }
